...
public class ShipMovementMyo : MonoBehaviour {
	...
	void Update () {
		...
		if (relativeRoll < -10f) {
			shipAnimator.SetInteger ("Direction", 6);
		} else if (relativeRoll > 10f) {
			shipAnimator.SetInteger ("Direction", 4);
		} else {
			shipAnimator.SetInteger ("Direction", 5);
		}
	}
